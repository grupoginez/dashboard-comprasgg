/* ════════════════════════════════════════════════════════════
   ...
   Construye ALL_DATA en tiempo real a partir de RAW_BY_SHEET
   (provisto por connector.js) y renderiza todas las secciones.
   Se ejecuta cada vez que se abre o cambia de pestaña/hoja.
   ════════════════════════════════════════════════════════════ */
...
let ALL_DATA = {};
let curSheet = 'MP';
let curCur = 'MXN', curEnt = 'ALL', isDark = true, filtersFrozen = false;
const F = {yr:[], mo:[], pv:[], pr:[]};
let sortKey='qty_total', sortDir=-1;
let pvChartType='bar', evChartType='line', rankChartType='horizontal', rankImpType='stacked', rankImpAxisMode='horizontal';
let selPriceMats = [];
...
// Transforma un registro crudo del connector al formato usado por el dashboard
function transformRecord(raw) {
  const cant_oc = raw.cantidad || 0;
  const cant_entregada = raw.cant_entregada || 0;
  const tieneEntrega = !!raw.fecha_entrega_real;

  // Periodo: basado en FECHA ENTREGA REAL. Si no hay entrega, 'PENDIENTE'
  let periodo, anio, mes, fecha_ent;
  if (tieneEntrega) {
    periodo = raw.fecha_entrega_real.slice(0, 7); // YYYY-MM
    anio = +raw.fecha_entrega_real.slice(0, 4);
    mes = +raw.fecha_entrega_real.slice(5, 7);
    fecha_ent = raw.fecha_entrega_real;
  } else {
    periodo = 'PENDIENTE';
    anio = raw.fecha ? +raw.fecha.slice(0,4) : null;
    mes = raw.fecha ? +raw.fecha.slice(5,7) : null;
    fecha_ent = '';
  }

  const var_cant = cant_entregada - cant_oc;
  const cumpl_pct = cant_oc > 0 ? (cant_entregada / cant_oc * 100) : (cant_entregada > 0 ? 100 : 0);
  const dias_ent = tieneEntrega ? diffDias(raw.fecha, raw.fecha_entrega_real) : null;
  ...
  return {
    ...
    um: raw.um,
    cantidad: cant_entregada,      // volumen ENTREGADO (eje principal)
    cant_oc,                        // volumen de la OC
    var_cant,
    cumpl_pct,
    dias_ent,
    ...
    moneda: raw.moneda,
    tc: raw.tc,
    importe: raw.importe_entregado || 0,
    total_neto: raw.total_neto,
    precio_u: raw.precio_unit || 0,
    cant_pendiente: raw.cant_pendiente,
    folio: raw.folio,
    entregado: tieneEntrega && cant_entregada > 0
  };
}

// Construye un sheet completo de ALL_DATA a partir de records ya transformados
function buildSheet(records) {
  ...
  const umSet = new Set();
  let usdTcSum = 0, usdTcCount = 0;

  records.forEach(r => {
    if (r.periodo && r.periodo !== 'PENDIENTE') periodsSet.add(r.periodo);
    if (r.proveedor) proveedoresSet.add(r.proveedor);
    if (r.producto) productosSet.add(r.producto);
    if (r.anio) yearsSet.add(r.anio);
    if (r.um) umSet.add(r.um);
    if (r.moneda === 'USD' && r.tc) { usdTcSum += r.tc; usdTcCount++; }
  });
  ...
  return {
    ...
    periods,
    periods_label,
    proveedores: [...proveedoresSet].sort(),
    productos: [...productosSet].sort(),
    years: [...yearsSet].sort((a,b)=>a-b),
    tc_avg_global: usdTcCount ? usdTcSum / usdTcCount : 1,
    um_present: [...umSet]
  };
}

// Punto de entrada: llamado por connector.js cuando RAW_BY_SHEET está listo
function initDashboard() {
  ALL_DATA = {};
  Object.keys(RAW_BY_SHEET).forEach(sheet => {
    const raws = RAW_BY_SHEET[sheet] || [];
    const records = raws.map(transformRecord);
    ALL_DATA[sheet] = buildSheet(records);
    const countEl = document.getElementById('stab-count-' + sheet);
    if (countEl) countEl.textContent = records.length.toLocaleString('es-MX');
  });
  rebuildFilters();
  refreshAll();
}

// Permite recargar datos manualmente sin volver a iniciar sesión
function reloadData() {
  const bar = document.getElementById('load-status-bar');
  if (bar) bar.textContent = 'Recargando datos desde SharePoint...';
  RAW_BY_SHEET = {};
  loadExcelData();
}

/* ════════════════════════════════════════════════════════════
   THEME / FREEZE / SECTIONS
   ════════════════════════════════════════════════════════════ */
...
/* ════════════════════════════════════════════════════════════
   CURRENCY / DELIVERY STATUS / FORMATTERS
   ════════════════════════════════════════════════════════════ */
function setCur(c){
  curCur=c;
...
function setEnt(e){
  curEnt=e;
  ['ALL','ENT','PEN'].forEach(x=>document.getElementById('ebtn-'+x).classList.toggle('active',x===e));
  refreshAll();
}
...
function resetAll(){
  ...
  curCur='MXN'; curEnt='ALL';
  ['MXN','USD','ALL'].forEach(x=>document.getElementById('cbtn-'+x).classList.toggle('active',x==='MXN'));
  ['ALL','ENT','PEN'].forEach(x=>document.getElementById('ebtn-'+x).classList.toggle('active',x==='ALL'));
  rebuildFilters();
  refreshAll();
}
...
function filtered(){
  return D().records.filter(r=>{
    ...
    if(curEnt==='ENT'&&!r.entregado) return false;
    if(curEnt==='PEN'&&r.entregado) return false;
    return true;
  });
}
...
function refreshAll(){
  ...
  if(subEl) subEl.textContent = (meta[curSheet]||curSheet) + ' · IMPORTE DE MAT. ENTREGADO · Fecha Entrega Real';
  const recs=filtered();
  renderChips();
  renderKPIs(recs);
  renderRanking(recs);
...
function renderKPIs(recs){
  ...
  const nPend=recs.filter(r=>!r.entregado).length;
  ...
  document.getElementById('kpis').innerHTML=`
    ...
    <div class="kpi r"><div class="kpi-l">OC Pendientes</div><div class="kpi-v">${nPend}</div><div class="kpi-s">sin entregar</div></div>`;
}
...
function renderMinMax(recs){
  const map={};
  recs.filter(r=>r.periodo!=='PENDIENTE').forEach(r=>{
  ...
  rows.forEach(r=>{h+=`<tr><td class="tl" title="${r.prod}">${r.prod}</td><td style="color:var(--acc2);font-weight:700">${fmt(r.mx.qty)}</td><td>${D().periods_label[r.mx.per]||r.mx.per}</td><td>${r.mx.prov}</td><td style="color:var(--acc3);font-weight:700">${fmt(r.mn.qty)}</td><td>${D().periods_label[r.mn.per]||r.mn.per}</td><td>${r.mn.prov}</td></tr>`;});
...
function renderEvolProv(recs){
  ...
  const yrs=[...new Set(recs.map(r=>r.anio).filter(y=>y!=null))].sort();
  dc('ch-evpv');
...
function renderPrices(recs){
  ...
  let h=`<thead><tr><th class="tl">Materia</th><th>Moneda</th><th>Precio Mín.</th><th>Precio Máx.</th><th>Prom. Pond.</th><th>Últ. Precio</th><th>Var. vs 1er precio</th><th>N Registros</th></tr></thead><tbody>`;
...
function renderOrdenes(recs){
  const recsConPeriodo=recs.filter(r=>r.periodo!=='PENDIENTE');
  const pm={};
  recsConPeriodo.forEach(r=>{ if(!pm[r.periodo]) pm[r.periodo]={f:new Set(),n:0}; if(r.folio&&r.folio!=='nan') pm[r.periodo].f.add(r.folio); pm[r.periodo].n+=r.importe; });
  const ps=Object.keys(pm).sort();
  const fv=ps.map(p=>pm[p].f.size);
  const tot=fv.reduce((a,b)=>a+b,0),mx=Math.max(...fv,0),mn2=Math.min(...fv,0),av=tot/(ps.length||1);
  document.getElementById('kpis-fo').innerHTML=`
    <div class="kpi"><div class="kpi-l">Total Órdenes</div><div class="kpi-v">${tot}</div></div>
    <div class="kpi g"><div class="kpi-l">Prom/Mes</div><div class="kpi-v">${av.toFixed(1)}</div></div>
    <div class="kpi y"><div class="kpi-l">Máximo</div><div class="kpi-v">${mx}</div><div class="kpi-s">${D().periods_label[ps[fv.indexOf(mx)]]||ps[fv.indexOf(mx)]||'—'}</div></div>
...
function renderTiempos(recs){
  ...
  const ps=Object.keys(pm2).filter(p=>p!=='PENDIENTE').sort();
  dc('ch-ttrend');
...
function renderEconomico(recs){
  ...
  const tm={};
  recs.filter(r=>r.periodo!=='PENDIENTE').forEach(r=>{ if(!tm[r.periodo]) tm[r.periodo]={mxn:0,usd:0}; if(r.moneda==='MXN') tm[r.periodo].mxn+=r.importe; else tm[r.periodo].usd+=r.importe; });
...
/* ════════════════════════════════════════════════════════════
   SHEET SWITCHER — construye en tiempo real al abrir/cambiar pestaña
   ════════════════════════════════════════════════════════════ */
